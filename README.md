## 더 효율적인 방식을 찾고, 실생활의 불편을 개선하고 싶은 개발자 이창헌입니다 😊

### 🌱 BackEnd Tool
![Static Badge](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Static Badge](https://img.shields.io/badge/SPRING%20BOOT-%236DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Static Badge](https://img.shields.io/badge/Spring_data_jpa-6DB33F?style=for-the-badge&logo=SpringSecurity&logoColor=white)
![Static Badge](https://img.shields.io/badge/Spring%20Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white)
![Static Badge](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Static Badge](https://img.shields.io/badge/-MongoDB-13aa52?style=for-the-badge&logo=mongodb&logoColor=white)
![Static Badge](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)

### 🎁 Infra Tool
![Static Badge](https://img.shields.io/badge/Amazon-AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Static Badge](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=Docker&logoColor=white)
![Static Badge](https://img.shields.io/badge/Jenkins-D24939?style=for-the-badge&logo=Jenkins&logoColor=white)

### 🍊 FrontEnd Tool
![Static Badge](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Static Badge](https://img.shields.io/badge/JAVASCRIPT-%23F7DF1E?style=for-the-badge&logo=javascript&logoColor=white)

### ⚙️ ETC
![Static Badge](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Static Badge](https://img.shields.io/badge/JIRA-%230052CC?style=for-the-badge&logo=jira&logoColor=white)

<br>

[![Solved.ac
프로필](http://mazassumnida.wtf/api/generate_badge?boj=changbill)](https://solved.ac/changbill)
<br>
<br>
<br>

## Development Experience
- 2023.07 ~ 2024.06 | SSAFY 10기 교육 수료 <br>
- 2024.06 ~ 2024.07 | 네이버부스트캠프 9기 베이직 <br>
- 2024.10 ~ 2024.11 | 우아한테크코스 7기 프리코스 <br>
<br>

## Achievements
- SSAFY 프로젝트 우수상 2회 수상 <br>
- 서울문화사 기획 공모전 최우수상 <br>
- 남양유업 대학생 마케팅 공모전 우수상 <br>
<br>

## License and Certificate
- 2024.12 | 정보처리기사 <br>
- 2023.10 | SQLD <br>
- 2022.04 | 컴퓨터활용능력2급 <br>
- 2025.05 | TOEIC 845 <br>
- 2025.12 | JLPT N3 <br>
<br>

## Site
✏️ [PORTFOLIO](https://gentle-condor-12a.notion.site/e8c7bf04d6198383a93701854f282143?pvs=74) <br/> 
📚 [TISTORY](https://poloopy.tistory.com/) <br/> 
<br>


### 개발 역량

#### 구현 내용
- Spring Framework의 MVC 패턴 적용, JPA 기반 도메인과 엔티티 연관관계 설계
- WebSocket, STOMP 기반 실시간 통신 기능 구현 경험
- Spring Security와 JWT를 활용한 인증·인가 구조 구현 경험
- JUnit 기반 테스트 코드 작성 및 기능 검증 경험
- Flutter FE를 맡아, REST API 엔드포인트를 기준으로 요청·응답 구조 파악, 인증 토큰과 데이터 화면에 연결
- Jenkins surge 슬롯을 활용한 점진적 Rolling 배포 구성 경험
- Nginx 기반 리버스 프록시 및 멀티 슬롯 로드밸런싱 구성 경험
- Cloudflare Tunnel 기반 홈서버 외부 노출 및 Zero Trust 환경 구성 경험

#### 개선 경험
- Pagination Fetch Join 트러블 슈팅 상세 조회와 목록 조회 DTO를 분리하여 해결, 응답시간 99.6% 개선
- EXPLAIN으로 옵티마이저의 실행 계획을 분석하고, 조회 조건과 정렬 기준에 맞는 복합 인덱스를 적용하여 <===========>
- Polling 방식의 메시지 기능을 WebSocket과 Redis Pub/Sub 기반 실시간 전파 구조로 개선
- 서비스 중단을 최소화하기 위해 로드밸런싱 기반의 SpringBoot 인스턴스를 2개 구성하고 점진적 롤링 배포 구현
- WAS 스케일 아웃 환경에서 세션 불일치 문제를 해결하기 위해 JWT 기반 인증을 적용하고, Redis를 활용해 서버 측 무효화 구조 구현
- 비관적 락으로 크레딧 거래 DB 동시성 문제 해결 및 Lock 순서를 통일해 DeadLock 문제 방지
- 동시성 제어 중 데이터 비갱신 문제, 트랜잭션 격리 수준 및 Fetch Join 조정하여 해결

#### ETC
- Git Flow 전략 기반 브랜치 관리 및 협업 경험
- Jira를 활용한 스프린트 기반 이슈 관리, Story Point 및 Epic 관리 경험
- Gerrit 기반 코드 리뷰 프로세스 경험
- Daily Scrum을 통한 진행 상황 공유 및 팀 커뮤니케이션 경험

#### AI / RAG / Agent
- n8n 기반 AI Agent RAG 시스템 설계 및 구현 경험
- 하네스 엔지니어링을 적용하여 계획, 구현, 검증 원칙을 정의하고, 오케스트레이터가 서브 에이전트에게 업무를 위임하는 구조 설계
- 작업 산출물 자동화, 중복 산출물 통합, 수정 범위 제한 등을 통해 AI 작업 효율 개선 경험

<br>
<br>
<br>
<br>
